package com.dobby.xiaohashu.user.relation.biz.consumer;
...
import com.dobby.xiaohashu.user.relation.biz.enums.FollowUnfollowTypeEnum;
import com.dobby.xiaohashu.user.relation.biz.model.dto.CountFollowUnfollowMqDTO;
...
public class FollowUnfollowConsumer implements RocketMQListener<Message> {
    ...
    private void handleFollowTagMessage(String bodyJsonStr) {
        ...
        boolean isSuccess = Boolean.TRUE.equals(transactionTemplate.execute(status -> {
            try {
                // 关注成功需往数据库添加两条记录
                // 关注表：一条记录
                int count = followDOMapper.insert(FollowDO.builder()
                        .userId(userId)
    ...
    private void handleUnFollowTagMessage(String bodyJsonStr) {
        ...
        boolean isSuccess = Boolean.TRUE.equals(transactionTemplate.execute(status -> {
            try {
                // 关注表：一条记录
                int count = followDOMapper.deleteByUserIdAndFollowingUserId(userId, unfollowUserId);
                // 粉丝表：一条记录
                if (count > 0) {
